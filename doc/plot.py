...
if __name__ == '__main__':
    ...
    # Next, let's plot queries 4 and 5 with and without an index on TYC1, BTmag.
    plt.figure()
    plt.subplot(121)
    ...
    for i, q in enumerate([n['q4a-tyci-3'], n['q5a-tyc1i-3'], n['q4a-bttyci-3'], n['q5a-bttyci-3']]):
        print(q[0])
        plt.bar([1 + 0.2 * i], q[0], 0.2)
    plt.xlabel('Neo4J'), plt.yscale('symlog', nonposy='clip', linthreshy=0.002)
    ...
    # Finally, let us plot query 1 with an index on TYC1, BTmag for clusters of different size.
    plt.figure()
    plt.subplot(131)
    for i, q in enumerate([c['q1-ni-1'], n['q1-bttyci-1']]):
        plt.bar([1 + 0.2 * i], q[0], 0.2)
    plt.xlabel('1 Node'), plt.yscale('symlog', nonposy='clip', linthreshy=0.002), plt.ylabel('Running Time (s)')
    plt.gca().set_ylim([0, 0.01]), plt.xticks([]), plt.subplot(132)
    for i, q in enumerate([c['q1-ni-2'], n['q1-bttyci-2']]):
        plt.bar([1 + 0.2 * i], q[0], 0.2)
    plt.xlabel('2 Nodes'), plt.yscale('symlog', nonposy='clip', linthreshy=0.002)
    plt.gca().set_ylim([0, 0.01]), plt.xticks([]), plt.yticks([]), plt.subplot(133)
    for i, q in enumerate([c['q1-ni-3'], n['q1-tyc1i-3']]):
        plt.bar([1 + 0.2 * i], q[0], 0.2)
    plt.xlabel('3 Nodes'), plt.yscale('symlog', nonposy='clip', linthreshy=0.002)
    plt.gca().set_ylim([0, 0.01]), plt.xticks([]), plt.yticks([])
    plt.subplots_adjust(wspace=0.0, left=0.16, right=0.98, bottom=0.12, top=0.97), plt.show()
